...
18. From **Adyen Payment Lifecycle Events** and **Adyen Event Entries**, **Open Source** opens the exact webhook request or report run. Repeat with retained content and with content marked as purged; metadata must remain reachable while the existing download action enforces retention.
